...
La quantité d'informations pouvant désormais être mis à la disposition de l'utilisateur est beaucoup plus importante (en particulier grâce aux seuils de visibilité et nouvelles thématiques). Côté administrateur, le confort pour créer et mettre à disposition des Analyses a également été amélioré.
...
- Des [seuils de visibilité]({{ site.url }}/fall-16#seuils-de-visibilit) pour que l'utilisateur accède à des visualisations différentes au niveau du département et du code postal
...
Notez également, que dans le panneau Analyses, les analyses cachées apparaissent en grisé (car elle ne sont pas disponibles au niveau de zoom actuel).
...
La possibilité de cloner des analyses est un gain de temps considérable pour les admistrateurs de l'application. Après avoir défini une première fois source de données et analyses, l'admnistrateur peut cloner le tout et mettre en place les mêmes analyses pour d'autres régions géographiques ou sur un segment des enregistrements de l'objet sur lequel il travaille. 
...
Les points d'intérêt permettent de répondre de façon simple et flexibles à de nombreux besoins des équipes commerciales.
...
Sans changer de carte, il a accès à une nouvelle section du panneau de gauche où il peut afficher des "Prospects" issus d'un objet Salesforce distinct. Il peut afficher les 200 prospects les plus proches du client qu'il quitte. Et comme pour ses Comptes, il a accès à leurs étiquettes et fiches information.
...
C'est tout pour le moment ! Rendez-vous le Mardi 13 Septembre 11h pour voir toutes ces nouveautés en action !

Ah, j'oubliais, si vous aussi vous vous posiez la question, [Galigeo for Salesforce fonctionne comme un charme dans Lightning](http://www.galigeo.com/fr/galigeo-salesforce-est-lightning-ready/).
